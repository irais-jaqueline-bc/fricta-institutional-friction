...
DATA_PATH = PROJECT_ROOT / "data" / "processed" / "fricta_scored.csv"
...
def load_data() -> pd.DataFrame:
    if not DATA_PATH.exists():
        raise FileNotFoundError(f"No se encontró el dataset canónico:\n{DATA_PATH}")
...
def validate_feature_columns(df: pd.DataFrame) -> None:
    missing = [feature for feature in FEATURE_SPEC if feature not in df.columns]
...
def audit_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature, specification in FEATURE_SPEC.items():
        ...
        outside_range = int(((series < 0) | (series > 1)).fillna(False).sum())
...
def main() -> None:
    ...
    correlation_matrix, high_correlations = calculate_correlations(df)
    exact_duplicates = find_exact_duplicates(df)

    audit_path = AUDIT_DIR / "icdm_feature_audit.csv"
    correlation_path = AUDIT_DIR / "icdm_feature_correlation_matrix.csv"
    high_correlation_path = AUDIT_DIR / "icdm_high_correlations.csv"
    duplicate_path = AUDIT_DIR / "icdm_exact_duplicate_features.csv"
    manifest_path = DESIGN_DIR / "feature_manifest_draft.csv"
    ...
    print("\n=== HIGH CORRELATIONS " f"|r| >= {HIGH_CORRELATION_THRESHOLD} ===\n")
