...
OUTPUT_ROOT = ROOT / "cipher" / "outputs" / "synthetic" / "counterfactual_smoke_v2"
...
AUDIT_PATH = (
    ...
    / "outputs"
    / "audit"
    / "stage8d2_counterfactual_smoke_audit_v2.json"
)
...
def main() -> None:
    ...
    print(
        "\n=== CIPHER STAGE 8D2 v2 — COUNTERFACTUAL / ROBUST-REACHABILITY SMOKE ===\n"
    )
    ...
    summary.to_csv(
        OUTPUT_ROOT / "stage8d2_v2_smoke_summary.csv",
        index=False,
    )

    gate_status = (
        "PASS_STAGE_8D2_V2_COUNTERFACTUAL_SMOKE"
        if all(technical_checks.values())
        else "FAIL_STAGE_8D2_V2_COUNTERFACTUAL_SMOKE"
    )
    ...
    if gate_status == "PASS_STAGE_8D2_V2_COUNTERFACTUAL_SMOKE":
        print(
