...
OUTPUT_ROOT = ROOT / "cipher" / "outputs" / "synthetic" / "performance_smoke"
AUDIT_PATH = (
    ROOT / "cipher" / "outputs" / "audit" / "stage8d1_core_pipeline_smoke_audit.json"
)
...
def run_scenario(
    ...
    if selected_k == 2:
        reference_profiles = remap_selected_labels_to_profiles(raw_selected_labels)
...
def main() -> None:
    ...
    print("\n=== CIPHER STAGE 8D1 — CORE PIPELINE PERFORMANCE SMOKE ===\n")
    print(
    ...
    summary.to_csv(
        OUTPUT_ROOT / "stage8d1_smoke_summary.csv",
        index=False,
    )

    report = {
        "status": (
            "PASS_STAGE_8D1_CORE_PIPELINE_SMOKE"
            if all(all_checks.values())
            else "FAIL_STAGE_8D1_CORE_PIPELINE_SMOKE"
        ),
    ...
    if report["status"] == "PASS_STAGE_8D1_CORE_PIPELINE_SMOKE":
        print(
