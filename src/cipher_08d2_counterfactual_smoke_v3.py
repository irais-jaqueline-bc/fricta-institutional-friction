...
OUTPUT_ROOT = ROOT / "cipher" / "outputs" / "synthetic" / "counterfactual_smoke_v3"
...
def select_diverse_pareto(
    ...
    for row in ordered:
        # Dict equality is unsafe here because each row contains
        # candidate_vector as a NumPy array. Membership via `row in selected`
        # triggers elementwise array comparison and can raise:
        # "truth value of an array is ambiguous".
        if id(row) in selected_object_ids:
            continue
...
def evaluate_s3_truth(
    ...
    # Numeric true_profile labels are post-hoc truth metadata only.
    # Swapping 1<->2 must not change oracle-reachability metrics or
    # latent-A/B direction recovery. Recompute the direction result explicitly
    # from the same frozen CF outputs and latent mapping.
    swapped_direction_recovered = (
        bool(predicted_source_latent == str(metadata["accessible_source_latent"]))
        if predicted_source_latent is not None
        else False
    )
    ...
    label_swap_invariant = bool(
        ...
        and swapped_direction_recovered == direction_recovered
    )
...
def single_model_summary_frame(
    results: list[dict[str, Any]],
) -> pd.DataFrame:
    rows = []
        ...
        rows.append(
            {
                ...
                "pareto_candidate_count": result["pareto_candidate_count"],
                "saved_candidate_count": len(result["saved_candidates"]),
                "single_model_reachable": bool(result["single_model_reachable"]),
            }
        )
...
def candidate_frame(
    ...
    if not rows:
        ...
        feature_columns = [f"candidate__{feature}" for feature in FEATURE_NAMES]
...
def run_applicable_scenario(
    ...
    if ensemble_report["eligible_members"] < MIN_ELIGIBLE_MEMBERS:
        insufficient_status = (
            "ROBUST_LAYER_NOT_EVALUABLE_INSUFFICIENT_ELIGIBLE_ENSEMBLE"
        )

        single_frame = single_model_summary_frame(single_results)
        ...
        # Preserve saved single-model candidates in a structurally valid CSV,
        # but robust-support columns remain absent because robust support was
        # not computed from an under-sized ensemble.
        ...
        for result in single_results:
            for candidate in result["saved_candidates"]:
                ...
                single_candidate_rows.append(row)

        if single_candidate_rows:
            single_candidate_frame = pd.DataFrame(single_candidate_rows)
        else:
            single_candidate_frame = pd.DataFrame(
                columns=[
                    "scenario",
                    "institution_id",
                    "source_profile",
        ...
        robust_pretruth = {
            ...
            "single_model_reachable_count": int(
                single_frame["single_model_reachable"].sum()
            ),
            "robust_reachable_tau_080_count": None,
            "robust_reachable_tau_090_count": None,
            "robust_reachable_tau_095_count": None,
            "robust_support_computed": False,
        }
        ...
        # Truth is accessed only after all single-model and ensemble-status
        # outputs above have been frozen.
        truth_evaluation = None
        ...
        if scenario == "S3_DIRECTIONAL_REACHABILITY":
            ...
            truth_merged = single_frame.merge(
                truth_subset,
                on="institution_id",
                how="left",
                validate="one_to_one",
            ...
            y_true = (
                truth_merged["oracle_reachable"].astype(bool).astype(int).to_numpy()
            )

            y_single = (
                truth_merged["single_model_reachable"]
                .astype(bool)
                .astype(int)
                .to_numpy()
            )
            ...
            truth_evaluation = {
                ...
                "robust_tau_090": "NOT_EVALUABLE",
                "direction_recovered": "NOT_EVALUABLE",
                "numeric_true_profile_label_swap_invariant": "NOT_EVALUABLE",
                "status": ("ROBUST_LAYER_NOT_EVALUABLE_INSUFFICIENT_ELIGIBLE_ENSEMBLE"),
            }

        elif scenario == "S6_NO_CLUSTER_NULL":
            truth_evaluation = {
                "false_robust_cf_claim_tau_090": None,
                "cf_applicable": True,
                "robust_layer_applicable": False,
                "queried_institutions": int(len(single_frame)),
            ...
            truth_evaluation = {
                ...
                "robust_layer_status": insufficient_status,
            }
...
def main() -> None:
    ...
    print(
        "\n=== CIPHER STAGE 8D2 v3 — COUNTERFACTUAL / ROBUST-REACHABILITY SMOKE ===\n"
    )
    ...
    for scenario in SCENARIOS:
        ...
        if (
            ...
            single_frame = single_model_summary_frame(result["query_results"])
            ...
            print(
                ...
                f"(< frozen minimum {MIN_ELIGIBLE_MEMBERS})",
            )

            print(
                "  single-model reachable:",
                f"{int(single_frame['single_model_reachable'].sum())}/20",
            )

            print(
                "  robust layer:",
                "NOT_EVALUABLE_INSUFFICIENT_ELIGIBLE_ENSEMBLE",
            )
            ...
            if scenario == "S3_DIRECTIONAL_REACHABILITY":
                ...
                print(
                    "  SMOKE robust metrics/direction:",
                    "NOT_EVALUABLE",
                )
    ...
    cf_processed = robust_completed + robust_non_evaluable

    technical_checks = {
        "prechecks_pass": all(prechecks.values()),
        "s1_s3_robust_layers_complete": {
            result["scenario"] for result in robust_completed
        }
        == {
            "S1_CONFIG_TWO_PROFILE",
            "S3_DIRECTIONAL_REACHABILITY",
        },
        "s6_gracefully_marked_robust_not_evaluable": any(
            result["scenario"] == "S6_NO_CLUSTER_NULL"
            and result["status"]
            == "ROBUST_LAYER_NOT_EVALUABLE_INSUFFICIENT_ELIGIBLE_ENSEMBLE"
            and result["ensemble_report"]["eligible_members"] < MIN_ELIGIBLE_MEMBERS
            and result["truth_evaluation"]["false_robust_cf_claim_tau_090"] is None
            for result in scenario_results
        ),
        ...
        "all_cf_processed_have_20_queries": all(
            len(result["query_results"]) == 20 for result in cf_processed
        ),
        "all_cf_processed_reference_partitions_reproduced": all(
            result["reference_partition_ari"] >= 0.999999 for result in cf_processed
        ),
        "all_ward_reference_fidelities_pass": all(
            result["reference_inductive_fidelity"] >= WARD_FIDELITY_MIN
            for result in cf_processed
            if (
                load_json(
                    CORE_SMOKE_ROOT
                    / result["scenario"]
        ...
        "robust_completed_ensembles_meet_minimum": all(
            result["ensemble_report"]["eligible_members"] >= MIN_ELIGIBLE_MEMBERS
            for result in robust_completed
        ),
        "non_evaluable_ensembles_are_below_minimum": all(
            result["ensemble_report"]["eligible_members"] < MIN_ELIGIBLE_MEMBERS
            for result in robust_non_evaluable
        ),
    ...
    for result in scenario_results:
        ...
        if result["status"] in {
            ...
            row["eligible_ensemble_members"] = result["ensemble_report"][
                "eligible_members"
            ]
                ...
                row.update(
                    {
                        ...
                        "robust_tau_080_n": int(
                            frame["robust_reachable_tau_080"].sum()
                        ),
                        "robust_tau_090_n": int(
                            frame["robust_reachable_tau_090"].sum()
                        ),
                        "robust_tau_095_n": int(
                            frame["robust_reachable_tau_095"].sum()
                        ),
                    }
                )
                ...
                row.update(
                    {
                        ...
                        "robust_tau_080_n": None,
                        "robust_tau_090_n": None,
                        "robust_tau_095_n": None,
                    }
                )
            ...
            if result["scenario"] == "S3_DIRECTIONAL_REACHABILITY":
                ...
                row["s3_oracle_positive_queries_smoke_only"] = truth[
                    "queried_oracle_positive_count"
                ]

                row["s3_single_model_f1_smoke_only"] = truth["single_model"]["f1"]
                ...
                if result["status"] == "COMPLETED":
                    ...
                    row.update(
                        {
                            "s3_robust_precision_smoke_only": None,
                            "s3_robust_f1_smoke_only": None,
                            "s3_direction_recovered_smoke_only": None,
                            "s3_label_swap_invariant_smoke_only": None,
                        }
                    )
    ...
    summary.to_csv(
        OUTPUT_ROOT / "stage8d2_v3_smoke_summary.csv",
        index=False,
    )

    gate_status = (
        "PASS_STAGE_8D2_V3_COUNTERFACTUAL_SMOKE"
        if all(technical_checks.values())
        else "FAIL_STAGE_8D2_V3_COUNTERFACTUAL_SMOKE"
    )
    ...
    if gate_status == "PASS_STAGE_8D2_V3_COUNTERFACTUAL_SMOKE":
        print(
