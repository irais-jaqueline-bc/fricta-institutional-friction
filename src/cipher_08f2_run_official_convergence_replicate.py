...
PLAN_PATH = ROOT / "cipher" / "design" / "stage8_official_run_plan_freeze_v2.json"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        help="Official convergence-audit replicate index, 3..11.",
    )
...
def compare_uncertainty(
    ...
    entropy_200 = left["normalized_entropy"].to_numpy(dtype=float)

    entropy_1000 = right["normalized_entropy"].to_numpy(dtype=float)
    ...
    if constant_200 or constant_1000:
        return {
            "status": "UNDEFINED_CONSTANT_INPUT",
            "normalized_entropy_spearman_200_vs_1000": None,
            "constant_entropy_200": constant_200,
            "constant_entropy_1000": constant_1000,
            "passes_0_95": False,
    ...
    if not np.isfinite(rho):
        return {
            "status": "UNDEFINED_NONFINITE_SPEARMAN",
            "normalized_entropy_spearman_200_vs_1000": None,
            "constant_entropy_200": constant_200,
            "constant_entropy_1000": constant_1000,
            "passes_0_95": False,
    ...
    return {
        ...
        "escalation_trigger": bool(rho < 0.95),
    }
...
def run_one_scenario(
    ...
    if list(governance_aux.columns) != [
        "institution_id",
        "governance_type",
    ]:
        raise RuntimeError(
            "Governance auxiliary interface contains unexpected columns."
        )
    ...
    technical_checks = {
        ...
        "uncertainty_convergence_complete_when_applicable": (
            ...
            or uncertainty_convergence["status"]
            in {
                "COMPLETED",
                "UNDEFINED_CONSTANT_INPUT",
                "UNDEFINED_NONFINITE_SPEARMAN",
            }
        ),
        "governance_auxiliary_interface_allowlisted": (
            list(governance_aux.columns)
            == [
                "institution_id",
                "governance_type",
            ]
...
def main() -> None:
    ...
    prechecks = {
        "stage8e1_plan_passed": (
            plan.get("gate_status") == "PASS_STAGE_8E1_CONVERGENCE_EDGE_SEMANTICS"
        ),
    ...
    print("\n=== CIPHER STAGE 8F2 — OFFICIAL CONVERGENCE-AUDIT REPLICATE ===\n")
    ...
    if not all(prechecks.values()):
        print("\nGATE STATUS: FAIL_STAGE_8F2_PRECHECK")
    ...
    for scenario in SCENARIOS:
        ...
        if unc["applicable"]:
            if unc["normalized_entropy_spearman_200_vs_1000"] is None:
                print(
    ...
    gate_status = (
        "PASS_STAGE_8F2_OFFICIAL_REPLICATE"
        if all(technical_checks.values())
        else "FAIL_STAGE_8F2_OFFICIAL_REPLICATE"
    )
    ...
    audit = {
        ...
        "latent_truth_used_pretruth": False,
        "governance_auxiliary_allowlist": [
            "institution_id",
            "governance_type",
        ],
        "undefined_spearman_policy": (
            "UNDEFINED_CONSTANT_INPUT => NA, pass=False, escalation_trigger=True"
        ),
    }
    ...
    if gate_status != "PASS_STAGE_8F2_OFFICIAL_REPLICATE":
        print(
